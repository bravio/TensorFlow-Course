...
# Generate some houses between 1000 and 3500 sqft
num_house = 160
np.random.seed(42)
...
plt.plot(house_size, house_price, "rx") # bx = blue x
plt.ylabel("Price")
plt.xlabel("Size")
plt.show()

# Normalize the values to prevent under or overflows
def normalize(array):
...
# Define ag radient descent optimizer that will minimize the loss defined in the
# cost operation
...
with tf.Session() as sess:
    ...
    for iteration in range(num_training_iter):
        ...
        # Display the current status to the console
    ...
    # Get values used to normalize data so we can denormalize back to its original state
    ...
    # Plot another graph that animates how Gradient Descent sequentially adjusts size_factor and price_offset to
    # find the values that return the "best" fit line
    fig, ax = plt.subplots()
